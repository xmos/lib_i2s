...
static void send(client tdm_callback_if tdm_i,
        out buffered port:32 p_dout[num_out],
        size_t num_out,
...
static void recieve(client tdm_callback_if tdm_i,
...
static void inline do_tdm(client tdm_callback_if tdm_i,
    ...
    t:> time;
    tdm_i.frame_start(time, restart);
    p_fsync <: fsync_mask[1];

    send(tdm_i, p_dout, num_out, channels_per_data_line, 1);

    for(unsigned frm_word_no = 2;frm_word_no < channels_per_data_line; frm_word_no++){
        recieve(tdm_i, p_din, num_in, channels_per_data_line, frm_word_no-2);
        p_fsync <: fsync_mask[frm_word_no];
        send(tdm_i, p_dout, num_out, channels_per_data_line, frm_word_no);
    }

    while(1){
        if (restart){
            recieve(tdm_i, p_din, num_in, channels_per_data_line, channels_per_data_line-2);
            recieve(tdm_i, p_din, num_in, channels_per_data_line, channels_per_data_line-1);
            sync(p_din[0]);
            p_fsync <: 0;
            return;
        }
        ...
        send(tdm_i, p_dout, num_out, channels_per_data_line, 0);

        t :> time;
        tdm_i.frame_start(time, restart);

        for(unsigned frm_word_no=1;frm_word_no < channels_per_data_line; frm_word_no++){
            recieve(tdm_i, p_din, num_in, channels_per_data_line,
...
void tdm_master_cb(client interface tdm_callback_if tdm_i,
    ...
    init_ports(p_dout, num_out, p_din, num_in, p_fsync, clk);
    ...
    while(1){
        ...
        tdm_i.init(offset, sclk_edge_count, channels_per_data_line);
        do_tdm(tdm_i,
            p_dout,num_out,
            p_din, num_in,
            p_fsync,
            offset, sclk_edge_count, channels_per_data_line);
