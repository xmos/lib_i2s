...
static void tdm_broadcast(unsigned sample_rate,
...
static void i2s_broadcast(unsigned mclk_freq, unsigned mclk_bclk_ratio,
        unsigned num_in, unsigned num_out, int is_i2s_justified){
    setup_strobe_port <: 0;
    ...
    while(1){
        ...
        case i2s_i.receive(size_t index, int32_t sample):{
          ...
          if (index < 8) {
            ...
            error |= (sample != rx_data[index][rindex]);
          }
          #else
          if (index >= 8) {
            if (sample != y)
        ...
        case i2s_i.init(i2s_config_t &?i2s_config, tdm_config_t &?tdm_config):{
          ...
          i2s_config.mclk_bclk_ratio = 4;
          i2s_config.mode = I2S_MODE_I2S;
          tdm_config.offset = 0;
          tdm_config.sync_len = 1;
          tdm_config.channels_per_frame = i2s_config.mclk_bclk_ratio * 2;
          frames_sent = 0;
          error = 0;
          first_time = 0;
          #ifdef I2S
          i2s_broadcast(mclock_freq[mclock_freq_index],
                        i2s_config.mclk_bclk_ratio, NUM_IN, NUM_OUT,
                        i2s_config.mode == I2S_MODE_I2S);
          #else
          tdm_broadcast(48000, 1, 1,
                        tdm_config.offset ==-1,
                        tdm_config.sync_len,
...
int main(){
    ...
    par {
        [[distribute]]
         app(i2s_i);
      i2s_tdm_master(i2s_i, p_dout, NUM_OUT, p_din, NUM_IN,
              p_bclk,
              p_lrclk,
              p_fsync,
              tdm_dout, 1,
